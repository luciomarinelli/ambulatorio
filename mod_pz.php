...
$qlistanamnesi="SELECT * FROM listanamnesi";
$result_qlistanamnesi=mysql_query($qlistanamnesi) or die (mysql_error());
echo "<div class=\"barranamnesi\">";
while ($listanamnesi=mysql_fetch_assoc($result_qlistanamnesi)) {
    $analink=$listanamnesi['link'];

    $qanamnesi="SELECT * FROM $analink WHERE idpz='$idpz'";
    $result_qanamnesi = mysql_query ($qanamnesi) or die (mysql_error());
    $anamnesipresente=mysql_num_rows($result_qanamnesi);

    echo "<span ";
    if ($anamnesipresente==1) echo "class=\"siscala\"";
    echo "><a href=javascript:popUp('$analink.php?idpz=$idpz&medico=$medico')>";
    echo $listanamnesi['descrizione'];
    echo "</a></span>  ";
...
$result_visite = mysql_query ($query_visite) or die (mysql_error());

	if ($result_visite) {
		while ($rigav = mysql_fetch_assoc($result_visite)) {
