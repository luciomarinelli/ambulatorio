...
//                                                                                                    //
// Questo programma Ë software libero; Ë lecito redistribuirlo e/o modificarlo secondo i termini      //
// della versione 2 della Licenza Pubblica Generica GNU come Ë pubblicata dalla Free Software         //
// Foundation. Questo programma Ë distribuito nella speranza che sia  utile, ma SENZA ALCUNA GARANZIA;//
// senza neppure la garanzia implicita di NEGOZIABILIT¿ o di APPLICABILIT¿ PER UN PARTICOLARE SCOPO.  //
...
// in caso contrario, se ne puÚ ottenere una scrivendo alla Free Software Foundation, Inc., 59 Temple //
...
//                                                                                                    //
// Il riferimento all'autore nella pagina in cui verr‡ sfruttato lo script non Ë obbligatorio ma      //
...
// oppure visitate il sito in cui questo script Ë implementato:                                       //
//                                                                                                    //
...
// propriet‡ $codicefiscale insieme ad eventuali errori nell'array errori[]                           //
//                                                                                                    //
//----------------------------------------------------------------------------------------------------//
...
//                                                                                                    //
// funzione principale da richiamare, restituisce un oggetto con le seguenti propriet‡:               //
//                  $r->codicefiscale:       risultato del calcolo                                    //
//                  $r->errori:              array contenente eventuali spiegazione di errori occorsi //
//                                                                                                    //
// - la data puÚ essere inviata sia in $cdatasingola in formato "gg/mm/aaaa" sia separatamente in     //
//   $cgiorno,$cmese,$canno                                                                           //
// - se $cdatasingola contiene qualcosa allora viene presa come data altrimenti vengono considerati   //
//   $cgiorno $cmese $canno                                                                           //
// - l'anno puÚ essere espresso in qualsiasi formato da 1 a 4 caratteri (es. 1,01,2001,987), il mese  //
//   ed il giorno possono essere sia a 2 che a 1 cifra (es. 5/6/1980 = 05/06/1980 = 5/06/1980)        //
// - se $csesso Ë F (come f) si considera femmina altrimenti per qualsiasi altro carattere maschio    //
//                                                                                                    //
// si puÚ richiamare la funzione in questi modi                                                       //
...
//----------------------------------------------------------------------------------------------------//
         ...
         $crisultato=new risultato; //conterr‡ risultato-errori

         //matrice per calcolare il carattere di controllo
         //il primo indice Ë il codice ascii del carattere il secondo indice la posizione pari o dispari
         // "A"
         $MATRICECOD[65][0] = 1;
         $MATRICECOD[65][1] = 0;
         // "B"
         ...
         //controlla in quale formato Ë stata passata la data
         if ($cdatasingola==""){
             //prende buoni $cgiorno $cmese $canno
             if (strlen($canno)<2) { $canno="0".$canno; }//se l'anno Ë 1 -> anno = 01
             //controlla validit‡ data
         ...
         if (!sizeof($crisultato->errori)) {
                  ...
                  //aggiunge lo 0 iniziale se $tmp Ë da 1 a 9                     //
                  if (strlen($tmp)<2){                                            //
                      $tmp="0".$tmp;                                              //
                  }                                                               //
                  $codice.=$tmp;                                                  //
...
//----------------------------------------------------------------------------------------------------//
         ...
         $i = 0;
         $stringa = "";//stringa conter‡ le 3 lettere
...
//----------------------------------------------------------------------------------------------------//
         ...
         if (strlen($cons)>3){
             //se ha 4 consonanti prende la 1∞, 3∞, 4∞
         ...
         $i = 0;
         //scorre il nome in cerca di vocali finchË $stringa non contiene 3 lettere
...
// Controlla la validit‡ di una data, restituendo true se la data Ë valida                            //
...
// La lista dei comuni Ë stata suddivisa in 4 files per rendere pi˘ veloce la ricerca.                //
// Se non viene trovato il comune il valore di ritorno Ë "0"                                          //
//                                                                                                    //
//----------------------------------------------------------------------------------------------------//
    ...
    // se l'iniziale non Ë una lettera ritorna il codice di errore
    ...
    $cod="0";
    // legge il file finchË non trova il comune (inserisce il codice in $cod)
...
//                                                                                                    //
/*                                                                                                    //
// - per utilizzare lo script Ë sufficiente inserire all'inizio della pagina in cui si effettua la    //
